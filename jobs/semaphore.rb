...
def update_builds(project, config)
  ...
  api_url = 'https://semaphoreapp.com/api/v1/projects?auth_token=' + config["semaphore_auth_token"]
...
SCHEDULER.every '2m', :first_in => 0  do
  unless config['projects'].empty?
    config['projects'].each do |project|
      send_event('semaphore', { items: update_builds(project, config) })
    end
  else
